<#
.SYNOPSIS
 Shows previous stats for teams based on BTTS
      ...
      forEach ($currentGame in $leagueStats)
      {
        ...
        if ($currentGame.Home -eq $homeTeamToCheck)
        {
          ...
          if (($overallTotalGames - $totalGames) -lt $numberOfGamesToCheck)
          {
            ...
            if (([int]$currentGame.hg -ge 1) -and ([int]$currentGame.ag -ge 1))
            {
              $overs += 1
            }
          }
...
Remove-Item ".\BTTS OtherLeagues AllTeams.csv" -Force -ErrorAction:SilentlyContinue
Remove-Item ".\BTTS OtherLeagues Fixtures.csv" -Force -ErrorAction:SilentlyContinue
...
if ($false){
  foreach ($league in $leagues)
  {
    ...
    foreach ($uniqueTeam in $uniqueTeams)
    {
      ...
      $line
      $line | Out-File .\"BTTS OtherLeagues AllTeams.csv" -Append -Encoding utf8
    } 
  }
}
...
foreach ($fixture in $fixtures)
{
  ...
  $line | Out-File .\"BTTS OtherLeagues Fixtures.csv" -Append -Encoding utf8
